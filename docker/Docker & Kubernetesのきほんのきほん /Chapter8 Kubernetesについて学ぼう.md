...
Kubernetesで使うPodやサービスに関する設定をマニフェストと呼ぶ。

記述したファイルをマニフェストファイルとと呼ぶ

形式はyaml形式。ファイルは任意の名前で良い。

p257

定義ファイルはリソース単位で記述する

- 初心者はリソースの項目として使うのは「サービス」と「デプロイメント」ぐらい
    - デプロイメントがレプリカセット、Podを内包しているため、「Pod」「レプリカセット」という項目は使わない
        - Podという項目は、Podのみを作成するときに使用する
